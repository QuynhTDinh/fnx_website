...
const BentoCard = ({ item, className, delay = 0 }) => (
  <Link to={item.link} className={`block relative rounded-2xl overflow-hidden group cursor-pointer border ${item.isAffiliate ? 'border-fnx-gold-dark/40 border-dashed' : 'border-white/10 hover:border-white/30'} transition-colors duration-500 ${className}`}>
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true }}
      transition={{ delay, duration: 0.8, ease: "easeOut" }}
      className="w-full h-full bg-[#0C141C]"
    >
      ...
      <div className="absolute inset-0 p-8 md:p-10 flex flex-col justify-end z-20">
        <div className="flex flex-col transform group-hover:-translate-y-2 transition-transform duration-300 ease-out">
          <h3 className={`text-3xl md:text-4xl font-sans font-bold ${item.isAffiliate ? 'text-fnx-gold-light' : 'text-white'} tracking-widest`}>
            {item.title}
          </h3>
...
export default function Ecosystem() {
  ...
  return (
    <section ref={containerRef} className="relative w-full py-24 bg-[#05080C] border-t border-white/5">
      <div className="text-center mb-20">
        <h2 className="text-4xl md:text-5xl font-sans font-bold text-white tracking-tight uppercase">Chuỗi Giá Trị <span className="text-fnx-gold-light">Thống Nhất</span></h2>
        <p className="text-fnx-silver mt-4 text-lg font-sans">Business Ecosystem</p>
      </div>
      ...
      <div className="relative w-full max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-1 flex flex-col gap-6 relative">
            ...
            <div className="absolute left-[-15px] top-10 bottom-10 w-[2px] bg-white/5 hidden md:block">
              <motion.div 
                className="absolute top-0 w-full h-32 bg-gradient-to-b from-transparent via-fnx-gold-light to-transparent"
                ...
                style={{ filter: 'drop-shadow(0 0 10px rgba(211,163,66,0.8))' }}
              />
            </div>

            <BentoCard item={bentoItems.rd} className="h-[250px]" delay={0.2} />
            <div className="hidden md:flex absolute left-[-23px] top-[125px] w-4 h-4 rounded-full border-2 border-fnx-gold-dark bg-black z-10 items-center justify-center">
               <div className="w-1.5 h-1.5 bg-fnx-gold-light rounded-full"></div>
            </div>

            <BentoCard item={bentoItems.sc} className="h-[250px]" delay={0.4} />
            <div className="hidden md:flex absolute left-[-23px] top-[395px] w-4 h-4 rounded-full border-2 border-fnx-gold-dark bg-black z-10 items-center justify-center">
               <div className="w-1.5 h-1.5 bg-fnx-gold-light rounded-full"></div>
            </div>

            <BentoCard item={bentoItems.m} className="h-[250px]" delay={0.6} />
            <div className="hidden md:flex absolute left-[-23px] top-[665px] w-4 h-4 rounded-full border-2 border-fnx-gold-dark bg-black z-10 items-center justify-center">
               <div className="w-1.5 h-1.5 bg-fnx-gold-light rounded-full"></div>
            </div>
